...
class Model(nn.Module):
    def __init__(self, in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=60,
                 embed_dim=96, num_people=2, num_frames=64, num_points=24, kernel_size=7, num_heads=32,
                 type_1_size=(8, 4), type_2_size=(8, 12), type_3_size=(8, 4), type_4_size=(8, 12),
                 attn_drop=0.5, head_drop=0., drop=0., rel=True, drop_path=0.2, mlp_ratio=4.,
                 act_layer=nn.GELU, norm_layer_transformer=nn.LayerNorm, index_t=True, global_pool='avg'):
    ...
    def forward(self, input):
        ...
        # index_t = torch.arange(60).to(torch.float)
        # index_t = F.interpolate(index_t[None, None, :], size=input.shape[2], mode='linear',
        #                         align_corners=False).squeeze()
        # index_t = 2 * index_t / input.shape[0] - 1
        index_t = torch.arange(64).to(torch.float)
        index_t = 2 * index_t / 64 - 1
        output = input.permute(0, 1, 2, 4, 3).contiguous().view(B, C, T, -1)  # [B, C, T, M * V]
        index_t = index_t.repeat(B, 1).to(output.device)
