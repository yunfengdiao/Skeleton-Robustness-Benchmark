...
from classifiers.skateformer.SkateFormer import Model
...
class SkateFormer(ActionClassifier):
    ...
    def createModel(self):
        ...
        if self.args.dataset == 'ntu60':
            Classifier = Model(num_classes=60, num_people=2)
        elif self.args.dataset == 'ntu120':
            Classifier = Model(num_classes=120, num_people=2)
        else:
            Classifier = Model(num_classes=65, num_people=1,type_1_size=(8, 4), type_2_size=(8, 6), type_3_size=(8, 4), type_4_size=(8, 6))
        # create the train data loader, if the routine is 'attack', then the data will be attacked in an Attacker
        if self.args.routine == 'train'or  self.args.routine == 'adTrain' or self.args.routine == 'finetune' or self.args.routine == 'attack' or self.args.routine == 'Aug_finetune' \
    ...
    def load_scheduler(self, n_iter_per_epoch):
        ...
        warmup_steps = int(25 * n_iter_per_epoch)
    ...
    def classificationLoss(self):
        self.classLoss = LabelSmoothingCrossEntropy(smoothing=0.1)
    ...
    def train(self):
        self.load_scheduler(len(self.trainloader))
        self.model.train()
