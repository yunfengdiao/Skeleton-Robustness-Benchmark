...
class BayesianAttacker(ActionAttacker):
    ...
    def build_mean_model(self, state_dict):
        cpy=copy.deepcopy(self.classifier.model)
        cpy.load_state_dict(state_dict)
        cpy.eval()
        return cpy

    def build_sqmean_model(self, state_dict):
        cpy = copy.deepcopy(self.classifier.model)
        cpy.load_state_dict(state_dict)
        cpy.eval()
        return cpy
    def attack(self):
        ...
        if self.args.classifier=='MSG3D'or self.args.classifier=='AGCN':
                self.args.trainedModelFile = 'swag_ep.pt'
                self.args.flag1=True
                self.classifier_mean = loadClassifier(self.args)
                mean_model_copy = copy.deepcopy(self.classifier_mean)

                self.args.flag2=True
                self.classifier_sqmean = loadClassifier(self.args)
                sqmean_model = copy.deepcopy(self.classifier_sqmean)

        else:
            swag_source = self.retFolder.rstrip('/BA/') + '/swag_ep.pt'
            state_dict = torch.load(swag_source)
            mean_model = self.build_mean_model(state_dict["mean_state_dict"])
            mean_model_copy = copy.deepcopy(mean_model)
            sqmean_model = self.build_sqmean_model(state_dict["sqmean_state_dict"])
        ...
        for batchNo, (tx, ty) in enumerate(self.classifier.trainloader):
            ...
            for ep in range(self.classifier.args.epochs):
                ...
                model_list = []
                num_model = self.args.n_models
                beta = 5e-30
                scale = self.args.scale
                if self.args.classifier=='MSG3D' or self.args.classifier=='AGCN':
                    for model_ind in range(num_model):
                        temp=copy.deepcopy(mean_model_copy)
                        temp1 = copy.deepcopy(mean_model_copy.model.model_joint).to(device)
                        temp2 = copy.deepcopy(mean_model_copy.model.model_bone).to(device)
                        noise_dict = OrderedDict()
                        #for joint
                        for (name, param_mean), param_sqmean, param_cur in zip(temp1.named_parameters(),
                                                                               sqmean_model.model.model_joint.parameters(),
                                                                               temp1.parameters()):
                        ...
                        for (name, param_cur), (_, noise) in zip(temp1.named_parameters(), noise_dict.items()):
                            param_cur.data.add_(noise, alpha=scale)

                        #for bone
                        for (name, param_mean), param_sqmean, param_cur in zip(temp2.named_parameters(),
                                                                               sqmean_model.model.model_bone.parameters(),
                                                                               temp2.parameters()):
                            var = torch.clamp(param_sqmean.data - param_mean.data ** 2, 1e-30)
                            var = var + beta
                            random_num = torch.randn_like(param_mean, requires_grad=False)
                            noise_dict[name] = var.sqrt() * random_num
                        for (name, param_cur), (_, noise) in zip(temp2.named_parameters(), noise_dict.items()):
                            param_cur.data.add_(noise, alpha=scale)

                        temp.model.model_joint=temp1
                        temp.model.model_bone=temp2
                        model_list.append(temp)


                else:
                    for model_ind in range(num_model):
                        temp = copy.deepcopy(mean_model_copy).to(device)
                        noise_dict = OrderedDict()
                        for (name, param_mean), param_sqmean, param_cur in zip(mean_model_copy.named_parameters(),
                        ...
                        for (name, param_cur), (_, noise) in zip(temp.named_parameters(), noise_dict.items()):
                            param_cur.data.add_(noise, alpha=scale)
                        model_list.append(temp)
                ...
                for model in model_list:
                    ...
                    if self.args.classifier=='MSG3D' or self.args.classifier=='AGCN':
                        pred=self.classifier.modelEval(adData)
                        if self.attackType == 'untarget':
                            loss = -torch.nn.CrossEntropyLoss()(pred, flabels)
                        else:
                            loss = torch.nn.CrossEntropyLoss()(pred, flabels)
                        ce_grad_sum += torch.autograd.grad(loss, [adData, ])[0].data
                        loss_sum += loss.item()
