...
import torch as K
import numpy as np
from Attackers.utils import MyAdam, create_logger,print_args,joint_corruption,pose_augmentation,temporal_cropresize,device
import os
import time
import random

class AugmentAttacker(ActionAttacker):
    ...
    def attack(self):
        ...
        for batchNo, (tx, ty) in enumerate(self.classifier.trainloader):
            ...
            for ep in range(self.classifier.args.epochs):
                adData.requires_grad = True
                adData_aug=adData.detach().clone()
                adData_aug.to(device)
                
                adData_aug = temporal_cropresize(adData_aug, [1,0.95])
                flip_prob  = random.random()
                if flip_prob < 0.5:
                        adData_aug = joint_corruption(adData_aug)
                else:
                        adData_aug = pose_augmentation(adData_aug)
                
                adData_aug.requires_grad = True  
                pred = self.classifier.modelEval(adData_aug)
                ...
                adData = adData.detach() - 0.01 * (cgs).sign()
        ...
        save_path3 = self.retFolder + folder + 'SMART_adv_data.pt'
